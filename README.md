...
PDF数据提取工具箱API，提供布局检测、OCR、公式检测与识别、表格解析等功能。

## 功能特点

- 布局检测：检测PDF文档中的布局元素如标题、段落、图像等
- OCR文字识别：识别图像中的文字内容
- 公式检测：检测文档中的数学公式位置
- 公式识别：识别数学公式内容并转换为LaTeX格式
- 表格解析：解析文档中的表格结构和内容
- PDF转Markdown：将PDF文档转换为Markdown格式
...
```bash
git clone https://github.com/yourusername/pdf-extract-kit-api.git
cd pdf-extract-kit-api
```
...
# 或者只下载特定模型
python src/download_models.py --models layout_detection formula_detection

# 可用的模型类型：
# - layout_detection：布局检测模型
# - formula_detection：公式检测模型
# - formula_recognition：公式识别模型
# - table_parsing：表格解析模型
```
...
- `POST /api/v1/upload` - 上传PDF文件或图像
- `POST /api/v1/layout-detection` - 检测文档布局
- `POST /api/v1/ocr` - 文字识别
- `POST /api/v1/formula-detection` - 公式检测
- `POST /api/v1/formula-recognition` - 公式识别
- `POST /api/v1/table-parsing` - 表格解析
- `POST /api/v1/pdf2markdown` - PDF转Markdown
- `POST /api/v1/run-project` - 通过配置运行项目

## 注意事项

所有API处理采用临时处理方式，不会在服务器上持久化存储用户上传的文件或处理结果。
